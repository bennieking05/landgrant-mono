...
from app.services.rules_engine import get_jurisdiction_config
...
class EdgeCaseAgent(BaseAgent):
    ...
    async def detect_edge_cases(self, parcel_id: str) -> EdgeCaseDetection:
        ...
        if (
            metadata.get("business_on_property")
            or metadata.get("property_type") == "commercial"
        ):
        ...
        if metadata.get("taking_type") == "partial" or metadata.get(
            "is_partial_taking"
        ):
        ...
        if jurisdiction == "MO":
            ...
            if family_years >= 50:
                ...
                alerts.append(
                    f"Heritage property: {family_years} years family ownership qualifies for 150% multiplier"
                )
        ...
        if ai_detection:
            for case_type in ai_detection.get("detected_edge_cases", []):
                if case_type not in detected:
                    ...
                    confidence[case_type] = ai_detection.get("confidence", {}).get(
                        case_type, 0.7
                    )
    ...
    async def handle_business_relocation(
        self, parcel_id: str
    ) -> BusinessRelocationResult:
    ...
    async def handle_partial_taking(self, parcel_id: str) -> PartialTakingResult:
        ...
        # Get valuations
        before_value = metadata.get("before_value", 0) or parcel_data.get(
            "appraisal_value", 0
        )
    ...
    async def handle_inverse_condemnation(self, parcel_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "recommended_action": (
                "File inverse condemnation claim" if is_inverse else "No action needed"
            ),
            "procedural_notes": (
                [
    ...
    async def _get_parcel_data(self, parcel_id: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            result = await self.db.execute(select(Parcel).where(Parcel.id == parcel_id))
    ...
    async def _ai_detect_edge_cases(
        ...
        prompt = EDGE_CASE_DETECTION_PROMPT.format(
            ...
            historical_context=str(
                parcel_data.get("metadata", {}).get("historical_context", {})
            ),
        )
    ...
    async def _calculate_goodwill(
        ...
        goodwill_allowed = config.compensation.get("business_goodwill", {}).get(
            "available", False
        )
    ...
    async def _analyze_remnant(
        ...
        if remainder_size < min_lot_size:
            is_uneconomic = True
            analysis.append(
                f"Remainder ({remainder_size:.0f} sqft) below minimum lot size ({min_lot_size} sqft)"
            )
        ...
        if before_value > 0:
            ...
            if value_ratio < 0.3:  # Less than 30% of original value
                is_uneconomic = True
                analysis.append(
                    f"Remainder value ({value_ratio:.1%}) is less than 30% of original"
                )
        ...
        analysis_text = (
            "; ".join(analysis) if analysis else "Remainder appears economically viable"
        )
    ...
    def _build_detection_result(
        ...
        return AgentResult(
            success=True,
            confidence=(
                max(detection.confidence.values()) if detection.confidence else 0.5
            ),
            data={
...
class BusinessRelocationHandler(BaseAgent):
    ...
    async def execute(self, context: AgentContext) -> AgentResult:
        result = await self.edge_case_agent.handle_business_relocation(
            context.parcel_id
        )
    ...
    async def calculate_goodwill(
        ...
        return {
            "goodwill": await self.edge_case_agent._calculate_goodwill(
                jurisdiction, business_info
            )
        }
...
class PartialTakingHandler(BaseAgent):
    ...
    async def calculate_severance_damages(
        ...
        after_value, severance, cost_to_cure = (
            await self.edge_case_agent._calculate_severance_damages(
...
class InverseCondemnationHandler(BaseAgent):
    ...
    async def execute(self, context: AgentContext) -> AgentResult:
        result = await self.edge_case_agent.handle_inverse_condemnation(
            context.parcel_id
        )
