...
def get_chroma_collection():
    ...
    if _collection is None and settings.rag_enabled:
        try:
            ...
            mode = (settings.rag_chroma_mode or "persistent").lower()
            if mode == "http":
                # Talk to the durable Chroma sidecar (see docker-compose).
                _chroma_client = chromadb.HttpClient(
                    host=settings.rag_chroma_host,
                    port=settings.rag_chroma_port,
                )
                logger.info(
                    "Connected to Chroma HTTP %s:%s",
                    settings.rag_chroma_host,
                    settings.rag_chroma_port,
                )
            else:
            ...
            logger.info(
                f"Initialized ChromaDB collection: {settings.rag_collection_name}"
            )
...
async def search(request: SearchRequest) -> list[RetrievalResult]:
    ...
    try:
        ...
        logger.debug(
            f"Search returned {len(retrieval_results)} results for: {request.query[:50]}..."
        )
...
async def ingest_rule_pack(
    jurisdiction: str, rule_data: dict[str, Any]
) -> dict[str, int]:
    ...
    _constitution = citations.get("constitution", "")
    ...
    if public_use:
        ...
        chunks.append(
            DocumentChunk(
                ...
                jurisdiction=jurisdiction,
                citation=(
                    citations.get("additional", [primary_citation])[0]
                    if citations.get("additional")
