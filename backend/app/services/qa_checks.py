...
STATE_REQUIRED_CLAUSES = {
    "TX": [
        {
            "id": "bill_of_rights_reference",
            "pattern": r"Landowner.*Bill.*Rights",
            "description": "Reference to Landowner Bill of Rights",
        },
        {
            "id": "property_code_citation",
            "pattern": r"Tex.*Prop.*Code.*§\s*21",
            "description": "Texas Property Code Chapter 21 citation",
        },
        {
            "id": "just_compensation",
            "pattern": r"(adequate|just)\s+compensation",
            "description": "Compensation language",
        },
    ],
    "FL": [
        {
            "id": "full_compensation",
            "pattern": r"full\s+compensation",
            "description": "Florida's 'full compensation' standard",
        },
        {
            "id": "fee_disclosure",
            "pattern": r"attorney.*fee",
            "description": "Attorney fee disclosure",
        },
    ],
    "CA": [
        {
            "id": "resolution_necessity",
            "pattern": r"resolution.*necessity",
            "description": "Resolution of Necessity reference",
        },
        {
            "id": "goodwill_notice",
            "pattern": r"business.*goodwill",
            "description": "Business goodwill compensation notice",
        },
    ],
    "MI": [
        {
            "id": "multiplier_disclosure",
            "pattern": r"125.*percent|1\.25",
            "description": "125% residence multiplier disclosure",
        },
        {
            "id": "fee_reimbursement",
            "pattern": r"(attorney|appraiser).*fee.*reimburs",
            "description": "Fee reimbursement notice",
        },
    ],
    "MO": [
        {
            "id": "heritage_multiplier",
            "pattern": r"(150|125).*percent|heritage|homestead",
            "description": "Heritage/homestead multiplier",
        },
    ],
}
...
FORBIDDEN_LANGUAGE = [
    {
        "pattern": r"waive.*all.*rights",
        "reason": "Cannot require waiver of all rights",
        "severity": "red",
    },
    {
        "pattern": r"final.*offer.*no.*negotiation",
        "reason": "Must allow good faith negotiation",
        "severity": "yellow",
    },
    {
        "pattern": r"confidential.*settlement",
        "reason": "Confidentiality clauses may be restricted",
        "severity": "yellow",
    },
    {
        "pattern": r"take.*it.*or.*leave.*it",
        "reason": "Coercive language",
        "severity": "red",
    },
]
...
class QACheckService:
    ...
    def check_document(
        ...
        citation_gate: Optional[Any] = None,
    ) -> QAReport:
        ...
        if context.get("amounts"):
            amount_results = self._check_amounts(document_content, context["amounts"])
        ...
        checks_failed = sum(
            1 for r in results if not r.passed and r.risk_level == "red"
        )
        checks_warned = sum(
            1 for r in results if not r.passed and r.risk_level == "yellow"
        )
        ...
        requires_counsel_review = (
            risk_level == "red"
            or len(required_clauses_missing) > 0
            or any(
                r.check_type == "forbidden_language" and not r.passed for r in results
            )
        )
        ...
        if requires_counsel_review:
            ...
            if required_clauses_missing:
                reasons.append(
                    f"Missing clauses: {', '.join(required_clauses_missing)}"
                )
        ...
        report = QAReport(
            ...
            or bool(
                getattr(citation_gate, "blocking", False)
        ...
        if citation_gate is not None:
            from app.services.citations import populate_qa_citation_counters

            populate_qa_citation_counters(report, citation_gate)
            if getattr(citation_gate, "blocking", False):
                report.risk_level = "red"
                report.passed = False
                if not report.escalation_reason:
                    report.escalation_reason = "Citation gate failed"
        ...
        for clause in clauses:
            ...
            results.append(
                QACheckResult(
                    ...
                    error_message=(
                        f"Missing required clause: {clause['description']}"
                        if not match
                        else None
                    ),
                    fix_suggestion=(
                        f"Add language referencing {clause['description']}"
                        if not match
                        else None
                    ),
                )
        ...
        for party in parties:
            ...
            results.append(
                QACheckResult(
                    ...
                    error_message=(
                        f"Name '{name}' not found in document"
                        if occurrences == 0
                        else None
                    ),
                    fix_suggestion=(
                        "Verify party name is correctly included"
                        if occurrences == 0
                        else None
                    ),
                )
        ...
        for date_name, date_value in dates.items():
            ...
            results.append(
                QACheckResult(
                    ...
                    error_message=(
                        f"Date {date_name} ({date_value}) not found"
                        if not found
                        else None
                    ),
                    fix_suggestion=(
                        "Verify date is correctly formatted" if not found else None
                    ),
                )
            )
    ...
    def _check_legal_description(
        ...
        results.append(
            QACheckResult(
                ...
                actual_value="Found" if found else "Not found",
                error_message=(
                    "Legal description does not match source documents"
                    if not found
                    else None
                ),
                fix_suggestion=(
                    "Verify legal description matches deed/title" if not found else None
                ),
            )
        )
        ...
        for amount_name, amount_value in amounts.items():
            ...
            results.append(
                QACheckResult(
                    ...
                    actual_value="Found" if found else "Not found",
                    error_message=(
                        f"Amount {amount_name} ({formatted}) not found"
                        if not found
                        else None
                    ),
                    fix_suggestion=(
                        "Verify amount matches appraisal/calculation"
                        if not found
                        else None
                    ),
                )
        ...
        for deadline_name, deadline_value in deadlines.items():
            ...
            results.append(
                QACheckResult(
                    ...
                    actual_value="Found" if found else "Not found",
                    error_message=(
                        f"Deadline {deadline_name} ({deadline_value}) not found"
                        if not found
                        else None
                    ),
                    fix_suggestion=(
                        "Verify deadline matches statutory requirements"
                        if not found
                        else None
                    ),
                )
...
def calculate_risk_score(
    ...
    return {
        ...
        "issues_summary": {
            ...
            "citation_issues": (
                citation_check.get("citations_invalid", 0) if citation_check else 0
            ),
        },
    }
