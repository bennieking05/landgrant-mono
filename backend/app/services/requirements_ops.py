...
class RequirementsOpsService:
    ...
    def __init__(self, storage_path: Optional[Path] = None):
        ...
        """
        self.storage_path = (
            storage_path or Path(__file__).resolve().parents[3] / "rules"
        )
    ...
    def _normalize_pack_to_requirements(
        self, pack_data: dict[str, Any], jurisdiction: str
        ...
        requirements.extend(
            self._extract_initiation_requirements(
                initiation, jurisdiction, version, base_citations
            )
        )
        ...
        requirements.extend(
            self._extract_compensation_requirements(
                compensation, jurisdiction, version, base_citations
            )
        )
        ...
        requirements.extend(
            self._extract_owner_rights_requirements(
                owner_rights, jurisdiction, version, base_citations
            )
        )
        ...
        if initiation.get("landowner_bill_of_rights"):
            reqs.append(
                NormalizedRequirement(
                    ...
                    citations=[
                        {
                            "citation": initiation.get(
                                "bill_of_rights_citation",
                                base_citations.get("primary", ""),
                            ),
        ...
        if initiation.get("initial_offer_days"):
            reqs.append(
                NormalizedRequirement(
                    ...
                    citations=[
                        {
                            "citation": base_citations.get("primary", ""),
                            "authority_level": "statute",
                        }
                    ],
                    authority_level="statute",
    ...
    def validate_pack(
        ...
        if jurisdiction and (len(jurisdiction) != 2 or not jurisdiction.isalpha()):
            errors.append(
                f"Invalid jurisdiction format: {jurisdiction} (expected 2-letter state code)"
            )
        ...
        for i, trigger in enumerate(pack_data.get("triggers", [])):
            ...
            if not trigger.get("match"):
                errors.append(
                    f"Trigger {trigger.get('id', i)} missing 'match' expression"
                )
            ...
            if match_expr and not self._is_valid_expression(match_expr):
                warnings.append(
                    f"Trigger {trigger.get('id')}: match expression may be invalid"
                )
        ...
        for i, chain in enumerate(pack_data.get("deadline_chains", [])):
            ...
            for j, deadline in enumerate(chain.get("deadlines", [])):
                ...
                if deadline.get("offset_days") is None:
                    errors.append(
                        f"Deadline {deadline.get('id', j)} missing 'offset_days'"
                    )
                if not deadline.get("citation"):
                    warnings.append(
                        f"Deadline {deadline.get('id', j)} missing citation"
                    )
    ...
    def publish_pack(
        ...
        if active_path.exists():
            archive_name = (
                f"{jurisdiction}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.yaml"
            )
                ...
                jurisdictions.append(
                    {
                        ...
                        "has_deadline_chains": len(pack_data.get("deadline_chains", []))
                        > 0,
                    }
                )
