...
async def _maybe_commit(db: object) -> None:
    """Commit a session that may be sync or async.

    The orchestrator started out expecting ``AsyncSession``; we now also
    accept the sync ``Session`` provided by ``app.api.deps.get_db``.  This
    helper bridges both without duplicating call sites.
    """
...
class AgentOrchestrator:
    ...
    async def execute_with_oversight(
        ...
        try:
            ...
            # 2. Log the decision
            decision_id = await self._log_decision(
                primary_agent, context, primary_result
            )

            # 3. Check if escalation is needed
            if primary_result.requires_review or primary_agent.should_escalate(
                primary_result
            ):
            ...
            # Log the failure
            decision_id = await self._log_decision(
                primary_agent, context, failure_result
            )
    ...
    async def _log_decision(
        ...
        decision_hash = sha256_hex(
            json.dumps(decision_data, sort_keys=True, default=str)
        )
        ...
        self.logger.info(
            f"Logged AI decision: {decision_id} (hash={decision_hash[:16]}...)"
        )
        return decision_id
    ...
    async def _cross_verify(
        ...
        """
        self.logger.info(
            f"Running cross-verification with {len(verification_agents)} agents"
        )
        ...
        valid_results = [r for r in verification_results if isinstance(r, AgentResult)]
        ...
        avg_verification_confidence = sum(verification_confidences) / len(
            verification_confidences
        )
