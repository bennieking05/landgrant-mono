...
class StateSummaryService:
    ...
    def get_clusters(self) -> list[StateCluster]:
        ...
        for state, config in self._state_configs.items():
            compensation = config.get("compensation", {})
            if compensation.get("residence_multiplier") or compensation.get(
                "heritage_multiplier"
            ):
    ...
    def get_state_delta(self, state: str) -> list[StateDelta]:
        ...
        compensation = config.get("compensation", {})
        owner_rights = config.get("owner_rights", {})
        _public_use = config.get("public_use", {})
    ...
    def get_state_summary(self, state: str) -> dict[str, Any]:
        ...
        return {
            ...
            "key_characteristics": {
                "quick_take_available": config.get("initiation", {})
                .get("quick_take", {})
                .get("available", False),
                "landowner_bill_of_rights": config.get("initiation", {}).get(
                    "landowner_bill_of_rights", False
                ),
                "compensation_base": config.get("compensation", {}).get(
                    "base", "fair_market_value"
                ),
                "attorney_fees_automatic": config.get("compensation", {})
                .get("attorney_fees", {})
                .get("automatic", False),
                "economic_development_banned": config.get("public_use", {}).get(
                    "economic_development_banned", False
                ),
                "jury_trial_available": config.get("owner_rights", {}).get(
                    "jury_trial", True
                ),
            },
    ...
    def export_markdown(self) -> str:
        ...
        for req in common:
            ...
            exceptions_str = (
                f" (except: {', '.join(req.exceptions)})" if req.exceptions else ""
            )
            lines.append(
                f"- **{req.requirement_id}**: {req.description} [{coverage}]{exceptions_str}"
            )
        ...
        for state in sorted(self._state_configs.keys()):
            ...
            lines.append(
                f"- Quick-take: {'Yes' if key_chars.get('quick_take_available') else 'No'}"
            )
            lines.append(
                f"- Bill of Rights: {'Yes' if key_chars.get('landowner_bill_of_rights') else 'No'}"
            )
            lines.append(f"- Compensation: {key_chars.get('compensation_base', 'FMV')}")
            lines.append(
                f"- Auto Attorney Fees: {'Yes' if key_chars.get('attorney_fees_automatic') else 'No'}"
            )
            lines.append(
                f"- Econ Dev Banned: {'Yes' if key_chars.get('economic_development_banned') else 'No'}"
            )
            lines.append("")
