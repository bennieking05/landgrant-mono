...
_WS_USER_ID_RE = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9._@-]{0,127}$")
_WS_RELAXED_ENVS = frozenset({"dev", "test"})


def _is_valid_ws_user_id(user_id: str) -> bool:
    if not user_id:
        return False
    if user_id == "anonymous":
        return False
    return bool(_WS_USER_ID_RE.match(user_id))
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str):
        ...
        logger.info(
            f"WebSocket connected: user={user_id}, total_connections={self.total_connections}"
        )
...
async def emit_offer_notification(
    ...
    notification = await create_notification(
        ...
        message=(
            f"{'Counter offer' if is_response else 'Offer'} for ${amount:,.0f}"
            if amount
            else f"New {offer_type} offer"
        ),
        data={
            "offer_id": offer_id,
            "parcel_id": parcel_id,
...
async def websocket_notifications(websocket: WebSocket):
    ...
    try:
        ...
        try:
            ...
            if settings.environment not in _WS_RELAXED_ENVS:
                if not _is_valid_ws_user_id(str(user_id)):
                    await websocket.send_json(
                        {
                            "type": "error",
