...
class OCRService:
    ...
    async def _process_with_document_ai(self, file_path: str) -> OCRResult:
        ...
        return OCRResult(
            text=document.text,
            confidence=(
                sum(p.confidence for p in document.pages) / len(document.pages)
                if document.pages
                else 0.0
            ),
            pages=len(document.pages),
            language=(
                document.pages[0].detected_languages[0].language_code
