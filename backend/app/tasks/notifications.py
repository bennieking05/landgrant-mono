...
def send_escalation_alert(
    self, escalation_id: str, recipient_ids: list[str], priority: str = "medium"
) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            body_parts = [
                "An AI agent decision requires your review.",
                "",
            ...
            body_parts.extend(
                [
                    "",
                    "Please review and take action in the LandGrant platform.",
                ]
            )
            ...
            for recipient_id in recipient_ids:
                user_result = db.execute(select(User).where(User.id == recipient_id))
                    ...
                    try:
                        result = preview_or_send(
                            ...
                            variables={
                                ...
                                "agent_type": (
                                    decision.agent_type if decision else "Unknown"
                                ),
                ...
                if priority in ["high", "critical"]:
                    ...
                    if phone:
                        try:
                            sms_body = f"LandGrant: {priority.upper()} escalation requires review. ID: {escalation_id[:8]}. Check your email for details."
                            result = preview_or_send(
                                channel="sms",
                                recipient=phone,
                                message=sms_body,
...
def send_compliance_violation_alert(
    self, case_id: str, violation_type: str, details: dict[str, Any]
) -> dict[str, Any]:
...
def send_document_ready_notification(
    ...
    try:
        ...
        try:
            ...
            doc_result = db.execute(select(Document).where(Document.id == document_id))
...
def send_deadline_reminder(
    ...
    try:
        ...
        try:
            ...
            dl_result = db.execute(select(Deadline).where(Deadline.id == deadline_id))
            ...
            urgency = (
                "urgent"
                if days_remaining <= 1
                else "warning" if days_remaining <= 3 else "reminder"
            )
            ...
            for recipient_id in recipient_ids:
                ...
                if days_remaining <= 1:
                    ...
                    if phone:
                        try:
                            sms_body = f"LandGrant URGENT: '{deadline.title}' due in {days_remaining} day(s). Take action now."
                            result = preview_or_send(
                                channel="sms",
                                recipient=phone,
                                message=sms_body,
