...
class GuardResult:
    ...
    @classmethod
    def allow(
        cls, reason: str = "All conditions met", confidence: float = 1.0
    ) -> GuardResult:
...
def guard_offer_sent_to_negotiation(
    ...
    days_remaining = (
        (deadline - datetime.utcnow()).days if offer.sent_date else response_days
    )
    return GuardResult.deny(
...
GUARD_CONDITIONS: dict[tuple[ParcelStage, ParcelStage], Callable] = {
    ...
    (
        ParcelStage.APPRAISAL,
        ParcelStage.OFFER_PENDING,
    ): guard_appraisal_to_offer_pending,
...
class WorkflowEngine:
    ...
    def get_valid_transitions(self, parcel: Parcel) -> list[ParcelStage]:
        ...
        current_stage = (
            ParcelStage(parcel.stage) if isinstance(parcel.stage, str) else parcel.stage
        )
    ...
    def execute_transition(
        ...
        try:
            ...
            current_stage = (
                ParcelStage(locked_parcel.stage)
                if isinstance(locked_parcel.stage, str)
                else locked_parcel.stage
            )
    ...
    def check_auto_progression(self, parcel: Parcel) -> Optional[TransitionResult]:
        ...
        for target_stage in valid_transitions:
            ...
            if (
                ...
                and guard_result.confidence >= self.confidence_threshold
            ):
        ...
        for parcel in query.all():
            ...
            for target_stage in valid_transitions:
                ...
                if guard_result.allowed and (
                    guard_result.requires_review
                    or guard_result.confidence < self.confidence_threshold
                ):
                    pending.append(
                        {
                            "parcel_id": parcel.id,
