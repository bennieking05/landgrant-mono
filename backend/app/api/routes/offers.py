    ...
    if payload.response_due_date:
        try:
            response_due = datetime.fromisoformat(
                payload.response_due_date.replace("Z", "")
            )
        except Exception as exc:
            raise HTTPException(
                status_code=422, detail="invalid_response_due_date"
            ) from exc
        ...
        "items": [
            {
                ...
                "source": o.source,
                "created_date": (
                    o.created_date.isoformat() + "Z" if o.created_date else None
                ),
                "sent_date": o.sent_date.isoformat() + "Z" if o.sent_date else None,
                "response_due_date": (
                    o.response_due_date.isoformat() + "Z"
                    if o.response_due_date
                    else None
                ),
                "response_date": (
                    o.response_date.isoformat() + "Z" if o.response_date else None
                ),
...
def get_offer(
    ...
    return {
        ...
        "created_date": (
            offer.created_date.isoformat() + "Z" if offer.created_date else None
        ),
        "sent_date": offer.sent_date.isoformat() + "Z" if offer.sent_date else None,
        "response_due_date": (
            offer.response_due_date.isoformat() + "Z"
            if offer.response_due_date
            else None
        ),
        "response_date": (
            offer.response_date.isoformat() + "Z" if offer.response_date else None
        ),
...
def update_offer(
    ...
    if payload.response_date is not None:
        try:
            offer.response_date = datetime.fromisoformat(
                payload.response_date.replace("Z", "")
            )
            changes["response_date"] = payload.response_date
        except Exception as exc:
            raise HTTPException(
                status_code=422, detail="invalid_response_date"
            ) from exc
...
def create_counteroffer(
    ...
    counter = models.Offer(
        ...
        terms_summary=payload.terms_summary,
        status=(
            OfferStatus.RECEIVED if payload.source == "landowner" else OfferStatus.DRAFT
        ),
...
def get_payment_ledger(
    ...
    return {
        ...
        "settlement_amount": (
            float(ledger.settlement_amount) if ledger.settlement_amount else None
        ),
        "settlement_date": (
            ledger.settlement_date.isoformat() + "Z" if ledger.settlement_date else None
        ),
        "payment_instruction_date": (
            ledger.payment_instruction_date.isoformat() + "Z"
            if ledger.payment_instruction_date
            else None
        ),
        "payment_cleared_date": (
            ledger.payment_cleared_date.isoformat() + "Z"
            if ledger.payment_cleared_date
            else None
        ),
...
def update_payment_ledger(
    ...
    if payload.settlement_date is not None:
        try:
            ledger.settlement_date = datetime.fromisoformat(
                payload.settlement_date.replace("Z", "")
            )
            changes["settlement_date"] = payload.settlement_date
        except Exception as exc:
            raise HTTPException(
                status_code=422, detail="invalid_settlement_date"
            ) from exc

    if payload.payment_instruction_date is not None:
        try:
            ledger.payment_instruction_date = datetime.fromisoformat(
                payload.payment_instruction_date.replace("Z", "")
            )
            changes["payment_instruction_date"] = payload.payment_instruction_date
        except Exception as exc:
            raise HTTPException(
                status_code=422, detail="invalid_payment_instruction_date"
            ) from exc

    if payload.payment_cleared_date is not None:
        try:
            ledger.payment_cleared_date = datetime.fromisoformat(
                payload.payment_cleared_date.replace("Z", "")
            )
            changes["payment_cleared_date"] = payload.payment_cleared_date
        except Exception as exc:
            raise HTTPException(
                status_code=422, detail="invalid_payment_cleared_date"
            ) from exc
