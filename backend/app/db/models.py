...
class Document(Base):
    ...
    # Phase 3.2 privilege & retention metadata.
    privilege_class = Column(
        String, default="work_product"
    )  # attorney_client | work_product | public | discovery_only
    retention_class = Column(
        String, default="default"
    )  # default | matter | litigation | regulatory
    retention_until = Column(DateTime)
    litigation_hold = Column(Boolean, default=False)
    metadata_json = Column(JSON, default=dict)
...
class AuditEvent(Base):
    ...
    # Hash chain: ``hash = sha256(prev_hash || canonical(payload))``.  Keeping
    # a nullable ``prev_hash`` lets us verify the entire log and detect
...
class PortalSession(Base):
    ...
    id = Column(String, primary_key=True)
    invite_id = Column(
        String, ForeignKey("portal_invites.id"), nullable=False, index=True
    )
...
class EsignEnvelope(Base):
    ...
    provider = Column(
        String, nullable=False, default="stub"
    )  # stub, docusign, hellosign
...
class ChatThread(Base):
    ...
    messages = relationship(
        "ChatMessage", backref="thread", cascade="all, delete-orphan"
    )
...
class Requirement(Base):
    ...
    )  # Canonical ID (e.g., "notice.initial_offer")
...
class PredictionOutcome(Base):
    """Ground-truth outcome for a settlement prediction.

    Written by ``ml_prediction.record_prediction_outcome`` when a case closes
    so the model accuracy pipeline (and any future Vertex AutoML training
    job) has a durable feature / label store.
    """
    ...
    model_used = Column(String, default="rules")
    input_features = Column(JSON, default=dict)
    predicted_settlement = Column(Numeric)
    predicted_low = Column(Numeric)
    predicted_high = Column(Numeric)
    predicted_confidence = Column(Numeric)

    actual_settlement = Column(Numeric)
    went_to_litigation = Column(Boolean, default=False)
    days_to_settlement = Column(Integer)
...
class RegulatoryUpdate(Base):
    ...
    change_type = Column(
        String, nullable=False
    )  # new_statute, amendment, repeal, court_ruling
    effective_date = Column(DateTime)
...
class CurativeItem(Base):
    ...
    item_type = Column(
        String, nullable=False
    )  # missing_heir, unreleased_lien, variance, etc.
