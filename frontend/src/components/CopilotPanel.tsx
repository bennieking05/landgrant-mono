    ...
    <div className="flex flex-col h-full bg-white border-l border-slate-200 relative">
      ...
      {(caseId || parcelId || jurisdiction) && (
        <div className="px-4 py-2 bg-slate-50 border-b border-slate-200 flex flex-wrap gap-2 text-xs">
          ...
          {caseId && (
            <span className="px-2 py-0.5 bg-brand/10 text-brand rounded-full">
              Case: {caseId}
            </span>
          )}
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                ...
                <button
                  onClick={() => setActiveQuickAction("explain")}
                  disabled={loading || !jurisdiction}
                  className="text-xs px-3 py-1.5 rounded-full bg-brand/10 border border-brand/30 text-brand hover:bg-brand/20 transition-colors disabled:opacity-50 flex items-center gap-1.5"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
