...
def engineer_features(inp: PredictionInput) -> dict[str, Any]:
    ...
    if (
        inp.previous_counter_offer
        and inp.counter_offer_amount
        and inp.assessed_value > 0
    ):
...
async def get_training_data(
    ...
    """Fetch training data from historical ``PredictionOutcome`` rows.

    Args:
        jurisdiction: Filter by jurisdiction (optional)
        min_records: Minimum records required
        db: SQLAlchemy session (sync or async); required for DB reads.

    Returns:
        List of training records.  Empty list when ``db`` is not supplied so
        callers can fall back to rules-based predictions cleanly.
    """

    if db is None:
        logger.info("get_training_data called without db; returning empty set")
        return []

    try:
        from app.db import models

        q = db.query(models.PredictionOutcome).filter(
            models.PredictionOutcome.actual_settlement.isnot(None)
        )
        if jurisdiction:
            q = q.filter(models.PredictionOutcome.jurisdiction == jurisdiction)
        rows = q.order_by(models.PredictionOutcome.outcome_date.desc()).all()
    except Exception as exc:
        logger.warning("training data fetch failed: %s", exc)
        return []
    ...
    for r in rows:
        records.append(
            TrainingRecord(
                features=r.input_features or {},
                actual_settlement=float(r.actual_settlement or 0),
                days_to_settlement=int(r.days_to_settlement or 0),
                went_to_litigation=bool(r.went_to_litigation),
                case_id=r.prediction_id,
                closed_date=r.outcome_date or r.created_at or datetime.utcnow(),
            )
        )

    logger.info(
        "training data: %d records (jurisdiction=%s, min=%d)",
        len(records),
        jurisdiction,
        min_records,
    )
...
async def prepare_training_dataset(
    ...
    Args:
        ...
        min_records: Optional override for the minimum-sample gate. Callers
            like :func:`run_training_loop` pass their own threshold so the
            loop's ``min_records`` argument is the single source of truth;
            when omitted we fall back to the global ML config default.
...
async def run_training_loop(
    ...
    *,
    ...
    """Pull outcomes from the DB, assemble a dataset and (optionally) train.

    The loop is idempotent and safe to call from Celery beat:

    1. Fetch :class:`TrainingRecord` rows via :func:`get_training_data`.
    2. If too few rows are available, return ``"status": "skipped"``.
    3. Otherwise build a dataset and, when Vertex / AutoML is configured,
       kick off :func:`train_model`.  Without Vertex the loop still returns
       the dataset summary so ops dashboards have visibility.
    """

    threshold = min_records or _ml_config.min_training_samples
    records = await get_training_data(
        jurisdiction=jurisdiction, min_records=threshold, db=db
    )

    if len(records) < threshold and not force:
        return {
            "status": "skipped",
            "reason": "insufficient_data",
            "records_available": len(records),
            "records_required": threshold,
        }

    dataset = await prepare_training_dataset(records, min_records=threshold)

    if not _ml_config.enabled or not _ml_config.project_id:
        return {
            "status": "prepared",
            "records": len(records),
            "feature_columns": dataset["feature_columns"],
            "note": "Vertex AI disabled; dataset prepared but not trained",
        }

    try:
        model_id = await train_model(dataset)
        return {
            "status": "trained",
            "records": len(records),
            "model_id": model_id,
...
def update_ml_config(
    ...
    logger.info(
        f"ML config updated: enabled={_ml_config.enabled}, endpoint={_ml_config.endpoint_id}"
    )
...
async def record_prediction_outcome(
    ...
    *,
    ...
    model_used: str = "rules",
    jurisdiction: Optional[str] = None,
    went_to_litigation: bool = False,
    days_to_settlement: Optional[int] = None,
    parcel_id: Optional[str] = None,
    project_id: Optional[str] = None,
) -> Optional[str]:
    """Persist a prediction outcome row used by the retraining loop.

    When ``db`` is not supplied the call becomes a no-op (logs only) so test
    code and offline evaluators can continue to call it freely.
    """

    logger.info(
        "prediction outcome: id=%s actual=$%s model=%s",
        prediction_id,
        actual_settlement,
        model_used,
    )
    ...
    try:
        ...
        row = models.PredictionOutcome(
            ...
            went_to_litigation=went_to_litigation,
        ...
        logger.warning("record_prediction_outcome persist failed: %s", exc)
        return None
...
async def calculate_model_accuracy(
    ...
    """Calculate aggregate accuracy metrics for ``model_type``.

    Computes MAE / MAPE / RMSE against the ``prediction_outcomes`` table
    restricted to the trailing ``window_days``.  Returns zeroed metrics when
    the DB is unavailable or the window is empty so callers can render the
    dashboard without special-casing first-boot.
    """
    ...
    try:
        ...
        q = (
            ...
            .filter(models.PredictionOutcome.model_used == model_type)
            .filter(models.PredictionOutcome.outcome_date >= cutoff)
            .filter(models.PredictionOutcome.actual_settlement.isnot(None))
            .filter(models.PredictionOutcome.predicted_settlement.isnot(None))
        )
        rows = q.all()
    except Exception as exc:
        logger.warning("calculate_model_accuracy failed: %s", exc)
        rows = []
