...
class PredictSettlementRequest(BaseModel):
    ...
    family_ownership_years: int = Field(
        0, ge=0, description="Years property held by family"
    )

    # Acquisition details
    partial_taking: bool = Field(
        False, description="Partial taking (not full acquisition)"
    )
    severance_impact: float = Field(
        0, ge=0, description="Estimated severance impact on remainder"
    )
    access_impact: bool = Field(False, description="Taking affects property access")
    business_on_property: bool = Field(
        False, description="Business operations on property"
    )

    # Dispute indicators
    owner_has_attorney: bool = Field(
        False, description="Owner has legal representation"
    )
    previous_counter_offer: bool = Field(False, description="Owner made counter-offer")
    counter_offer_amount: Optional[float] = Field(
        None, description="Amount of owner's counter-offer"
    )
    owner_contested_appraisal: bool = Field(
        False, description="Owner contested appraisal"
    )
...
async def predict_case_settlement(
    request: PredictSettlementRequest, persona: Persona = Depends(get_current_persona)
):
    ...
    property characteristics.
    """
    authorize(persona, "analytics", Action.READ)
    try:
...
async def get_case_risk_profile(
    ...
    try:
        ...
        if parcel_id:
            parcel = (
                db.query(models.Parcel).filter(models.Parcel.id == parcel_id).first()
            )
            if not parcel:
                raise HTTPException(
        ...
        counter_offers = [
            o for o in offers if o.offer_type and "counter" in str(o.offer_type).lower()
        ]
        if counter_offers:
            factors.append(
                RiskFactorModel(
        ...
        if lit_cases:
            ...
            recommendations.append(
                "Coordinate with outside counsel on litigation strategy"
            )
        ...
        if parcel_id:
            ...
            if not appraisal or not appraisal.completed_at:
                ...
                total_risk += 10
                recommendations.append(
                    "Complete appraisal before proceeding with offer"
                )
...
async def suggest_counter_offer(
    request: CounterOfferRequest, persona: Persona = Depends(get_current_persona)
):
...
async def get_jurisdiction_stats(
    jurisdiction: str, persona: Persona = Depends(get_current_persona)
):
...
async def list_property_types(persona: Persona = Depends(get_current_persona)):
...
async def list_project_types(persona: Persona = Depends(get_current_persona)):
