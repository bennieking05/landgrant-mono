...
def generate_factors(
    inp: PredictionInput, jurisdiction_config
    ...
    if initiation.get("landowner_bill_of_rights"):
        risk_factors.append(
            "State requires Landowner Bill of Rights - strict procedural compliance needed"
        )
    ...
    if inp.owner_occupied and inp.principal_residence:
        mult = get_compensation_multiplier(
            inp.jurisdiction,
            {
                "owner_occupied": True,
        ...
        if mult > 1.0:
            risk_factors.append(
                f"Owner-occupied residence multiplier applies ({mult:.0%})"
            )
    ...
    if inp.owner_has_attorney:
        risk_factors.append(
            "Owner has legal representation - expect formal negotiations"
        )
    ...
    if not inp.owner_has_attorney:
        factors.append(
            "Owner not represented by attorney - potential for quicker settlement"
        )
    ...
    if attorney_fees.get("automatic"):
        risk_factors.append(
            "Automatic attorney fee recovery for owner - litigation incentive"
        )
...
def generate_strategy(
    ...
    if dispute_level == DisputeLevel.LOW:
        ...
        ceiling_pct = 1.05
        strategy = (
            "Direct approach - make competitive initial offer close to fair value"
        )
...
def predict_settlement(inp: PredictionInput) -> SettlementPrediction:
    ...
    # Generate strategy
    initial_offer, ceiling, strategy = generate_strategy(inp, dispute_level, expected)
...
def compute_risk_profile(
    ...
    spread = (
        prediction.high_settlement - prediction.low_settlement
    ) / prediction.expected_settlement
    if spread > 0.3:
        financial_risk += 15
        factors.append(
            {
    ...
    if prediction.litigation_probability > 0.4:
        ...
        litigation_indicators.append(
            f"Litigation probability at {prediction.litigation_probability:.0%}"
        )
    ...
    if inp.owner_contested_appraisal:
        ...
        recommendations.append(
            "Consider ordering second appraisal or review appraisal methodology"
        )
