    ...
    try:
        ...
        if request.parcel_id:
            comms_query = comms_query.filter(
                models.Communication.parcel_id == request.parcel_id
            )
        ...
        if request.parcel_id:
            rules_query = rules_query.filter(
                models.RuleResult.parcel_id == request.parcel_id
            )
        ...
        manifest = {
            ...
            "parcel": (
                {"id": parcel.id, "county_fips": parcel.county_fips} if parcel else None
            ),
            "communications": [
                {
                    "id": c.id,
        ...
        db.rollback()
        _logger.error(
            f"Binder export failed for project {request.project_id}: {e}", exc_info=True
        )
...
def get_valid_transitions(
    ...
    return {
        "parcel_id": parcel_id,
        "current_stage": (
            parcel.stage.value if hasattr(parcel.stage, "value") else parcel.stage
        ),
...
def execute_transition(
    ...
    if request.skip_guards and persona not in [
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid stage: {request.target_stage}"
        )
    ...
    return [
        PendingItem(
            ...
            current_stage=(
                ...
                if hasattr(item["current_stage"], "value")
                else item["current_stage"]
            ),
...
def trigger_workflow_event(
    ...
    parcel = (
        db.query(models.Parcel).filter(models.Parcel.id == request.parcel_id).first()
    )
    if not parcel:
        raise HTTPException(
            status_code=404, detail=f"Parcel not found: {request.parcel_id}"
...
def get_parcel_history(
    ...
    history = (
        ...
        .order_by(StatusChange.occurred_at.desc())
        .limit(limit)
        .all()
    )
    ...
    return [
        HistoryItem(
            ...
            reason=item.reason,
            actor_persona=(
                item.actor_persona.value
                if hasattr(item.actor_persona, "value")
                else str(item.actor_persona)
            ),
...
def resolve_escalation(
    ...
    escalation = (
        db.query(WorkflowEscalation)
        .filter(WorkflowEscalation.id == escalation_id)
        .first()
    )
    if not escalation:
        raise HTTPException(
            status_code=404, detail=f"Escalation not found: {escalation_id}"
        )

    if escalation.status != "pending":
        raise HTTPException(
            status_code=400, detail=f"Escalation already resolved: {escalation.status}"
        )
    ...
    if action == "approve":
        ...
        parcel = (
            ...
            .filter(models.Parcel.id == escalation.parcel_id)
            .first()
        )
        if parcel and escalation.target_stage:
            ...
            result["to_stage"] = (
                transition_result.to_stage.value if transition_result.to_stage else None
            )
    ...
    escalation.resolved_by = (
        str(persona.value) if hasattr(persona, "value") else str(persona)
    )
