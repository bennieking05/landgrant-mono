...
class ValuationAgent(BaseAgent):
    ...
    def _calculate_discrepancy(
        ...
        return (
            (appraisal_value - avm_result.consensus_value) / avm_result.consensus_value
        ) * 100
        ...
        try:
            ...
            if parcel:
                ...
                return {
                    ...
                    "is_partial_taking": parcel.metadata_json.get(
                        "is_partial_taking", False
                    ),
                    "owner_occupied": parcel.metadata_json.get("owner_occupied", False),
                    "principal_residence": parcel.metadata_json.get(
    ...
    async def _calculate_severance(self, parcel_data: dict[str, Any]) -> float:
        """Calculate severance damages for a partial taking.

        Applies a before-and-after model scaled by taking geometry, access
        impacts, and utility-corridor penalties.  Values marked ``unknown``
        default conservatively so counsel gets a higher number to review
        rather than a misleadingly low one.
        """

        before_value = float(parcel_data.get("appraisal_value") or 0)
        if before_value <= 0:
            return 0.0

        taking_percent = float(parcel_data.get("taking_percent") or 30)
        taken_value = before_value * (taking_percent / 100.0)

        # Base severance: 15% of taken value covers typical access/visibility loss.
        severance = taken_value * 0.15

        # Access penalty: loss of primary frontage / driveway.
        if parcel_data.get("access_impaired"):
            severance += before_value * 0.05

        # Utility corridor / easement across remainder.
        if parcel_data.get("utility_corridor"):
            severance += taken_value * 0.10

        # Irregular remainder shape reduces marketability.
        if parcel_data.get("irregular_remainder"):
            severance += taken_value * 0.05

        # Cap at 50% of before-value so the agent doesn't return absurd numbers.
        return min(severance, before_value * 0.5)
